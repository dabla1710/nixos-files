      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          ...
          data.markets.forEach((market) => {
            MarketplaceSource._createPricesHandler(market, appId)(response, items)
              .then(resolve)
              .catch(reject);
          })
  ...
  _createPricesHandler: (market, appid) => async ({success, items}, hashNames) => {
    if (success) {
      ...
      const result = hashNames.reduce((acc, val) =>  (
        { ...acc,
          [val]: { [market]: {price: items[val] && items[val][market] ? items[val][market].price : null, priceF: items[val] && items[val][market] ? items[val][market].priceF : null, priceB: items[val] && items[val][market] ? items[val][market].priceB : null}, }
        }), {});
  ...
  GetPrices: async (appid, items, show, callback = () => {
    ...
    try {
      ...
      namesChunks.forEach((names, i) => {
        promises.push(SteamBySihSource._getPrices({...data, names}, true)
          .then(async (success) => {
    ...
    const names = Object
      ...
      .filter(key => {
        if (SteamBySihSource._cache && SteamBySihSource._cache[market][appid]) {
          const findIndex = Object.keys(SteamBySihSource._cache[market][appid]).findIndex(cache_hash_name => (cache_hash_name === descs[key].market_hash_name || (descs[key].description && cache_hash_name === descs[key].description.market_hash_name)));
          if (findIndex === -1) {
