...
BuyCardSetDialog = {
    ...
    OnReload: function (event) {
        ...
        $J('.badge_card_to_collect').each(function (i, e) {
          ...
          var priceLink = window.location.protocol + '//steamcommunity.com/market/priceoverview/?appid=' + appID + '&country=' + COUNTRY + '&currency=' + steamCurrency.eCurrencyCode + '&market_hash_name=' + marketHashname;
        ...
        $J('.badge_card_set_card.owned').each(function (i, e) {
            ...
            var priceLink = window.location.protocol + '//steamcommunity.com/market/priceoverview/?appid=' + 753 + '&country=' + COUNTRY + '&currency=' + steamCurrency.eCurrencyCode + '&market_hash_name=' + marketHashname;
            var row = $J(`<div class="market_listing_row market_recent_listing_row" data-name="${marketHashname}">`);
            row.append('<div class="market_listing_item_img_container"><img src="' + imgUrl + '" class="market_listing_item_img" /></div>');
            row.append('<div class="market_listing_right_cell market_listing_their_price">' +
                '<div style="float: right"><input type="number" value="" placeholder="Loading" min="0.03" step="0.01" class="market_dialog_input sih-card-price" style="width: 60px; text-align: right; padding-right: 3px; margin-top: 5px; background: #1a1a1a; border: rgb(112, 112, 112) 1px solid; color: #909090;" /></div></div>');
            ...
            RequestCacher.get({
                ...
                if (altName.indexOf('(Trading Card)') == -1) {
                    ...
                    RequestCacher.get({
                        method: "GET",
                        url: '//steamcommunity.com/market/priceoverview/?appid=753&country=' + COUNTRY + '&currency=' + steamCurrency.eCurrencyCode + '&market_hash_name=' + altName
                    }).then((response) => {
                        var lp = 0;
                        if (response.success) {
                            lp = response.lowest_price;
    ...
    CalculateTotal: function () {
      ...
      $J('#market_buycardset_dialog_buyall span').text(SIHLang.tradingcards.buyall + ' (' + BuyCardSetDialog.m_total.toFixed(2) + ')');
    },
...
function GetPriceValueAsInt(strAmount) {
  ...
  strAmount = strAmount.replace(GetCurrencySymbol(GetCurrencyCode(g_rgWalletInfo ? g_rgWalletInfo['wallet_currency'] : steamCurrency.eCurrencyCode)), '').replace(',', '.').replace('.--', '.00');
