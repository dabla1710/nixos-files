...
CAppwideInventory.prototype.LoadCompleteInventory = function() {
  ...
  if ( !this.m_promiseLoadCompleteInventory )
  {
    ...
    var _this = this;
    this.m_promiseLoadCompleteInventory = $J.when.apply( $J, rgDeferreds ).done( function() {  _this.m_bFullyLoaded = true; SIH?.itemCard?.ItemCardModify(); } );
  }
      ...
      const updateFloatValue = function (data) {
        ...
        if (Array.isArray(CSGO_ORIGINS) && CSGO_ORIGINS.length && data.iteminfo) {
          ...
          if (data.iteminfo.globalRatingPos || data.iteminfo.localRatingPos) {
            ...
            $floatBlock.find('.item-position').css('display', 'inline-flex').tooltip({
              content: function () {
  ...
  try {
    ...
    if (it.length > 0) {
      ...
      if (rgItem.goo_value > 0) {
        ...
        $J(`#${appid}_${contextid}_${cellId}.selectedSell`)
...
var ModifySellingFunctions = function () {
  ...
  $J('#ck_autoadjust').change(function () {
    ...
    if (!isChecked) {
      ...
      if (g_ActiveInventory.selectedItem.description.buyerPrice && g_ActiveInventory.selectedItem.description.sellerPrice) {
        recalcPrice();
      }
      else {
        GetQueuePrice(cb => {
...
const updateInventoryProviders = () => {
  ...
  const providersData = window.PROVIDERS_LIST.filter(x => allCurrentSources.includes(x.id) && !['backpacktf'].includes(x.id));
    ...
    $J.each(ExternalPrices[appid].apis, (idx, elem) => {
      ...
      if (elem.api && elem.api.GetPrices && curSources.includes(it.id)) {
       ...
       timer = setInterval(() => {
         ...
         if ($itemPriceContainer === (g_ActiveInventory.m_cItems || Object.keys(g_ActiveInventory.m_rgDescriptions).length)) {
            clearInterval(timer);
            cnt += 1;
