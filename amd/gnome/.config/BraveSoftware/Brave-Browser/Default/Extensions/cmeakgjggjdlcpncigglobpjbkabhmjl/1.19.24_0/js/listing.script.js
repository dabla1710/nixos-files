...
var setFloatValue = function (data) {
  ...
  if (data && data.success && $item[0]) {
    ...
    if (data.iteminfo.globalRatingPos || data.iteminfo.localRatingPos) {
      floatDiv.find('.item-position').css('display', 'inline-flex').tooltip({
        content: function () {
      ...
      if (data.iteminfo.localRatingPos) {
        floatDiv.find('.item-position .item-position__local').css('display', 'flex').find('.value').html(`${data.iteminfo.localRatingPos}`);
      }
...
var setStickersWearValues = function (data) {
  ...
  const { isRenderStickerPrice = true } = JSON.parse(localStorage.getItem('SIH_CONTROL_PANEL_MARKET_LISTINGS')) || { isRenderStickerPrice };
    ...
    return AjaxMaster(req)
      .success(res => {
        if (req.url.includes('/market/itemordershistogram')) {
          ...
          const statDay = +$J('.stat_day').html().replace(/&nbsp;/g, '');
          const statWeek = +$J('.stat_week').html().replace(/&nbsp;/g, '');
          const statMonth = +$J('.stat_month').html().replace(/&nbsp;/g, '');
          ...
          SIH?.SihUserPrices?.sendItemData(res, +appId, encodedHashName, +req.data.currency, steamSoldData, req.data.language);
