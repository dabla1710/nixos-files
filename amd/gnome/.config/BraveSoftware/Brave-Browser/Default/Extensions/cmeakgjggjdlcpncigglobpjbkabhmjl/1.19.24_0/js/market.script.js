...
const parseListings = function () {
  ...
  const totalSellingStr = totalSelling ? `<span class="sih-total-sell__selling text text_white fs-14" data-price="${totalSelling / 100}">${formatNumber(totalSelling / 100)}</span>` : '';
  const totalRecieveStr = totalRecieve ? `<span class="sih-total-sell__receive text text_grey-900 fs-12" data-price="${totalRecieve / 100}">(${formatNumber(totalRecieve / 100)})</span>` : '';
  ...
  if (!$J('#totalRow0').find('.sih-total-sell').length) {
    ...
    $J('#totalRow0').find('.sih-total-sell__selling').attr('data-price', `${totalSelling / 100}`).text(`${formatNumber(totalSelling / 100)}`);
    $J('#totalRow0').find('.sih-total-sell__selling').attr('data-price', `${totalRecieve / 100}`).text(`(${formatNumber(totalRecieve / 100)})`);
    ...
    if (typeof (window.totalrow) === 'undefined' || window.totalrow) {
      ...
      `);
        ...
        if (!$listingsTable.find(`#totalRow${idx}`).length) {
          ...
          $listingsTable.find('.sih-market-total__seller').attr('data-price', `${totalSelling / 100}`).text(`${formatNumber(totalSelling / 100)}`);
          $listingsTable.find('.sih-market-total__receive').attr('data-price', `${totalRecieve / 100}`).text(`(${formatNumber(totalRecieve / 100)})`);
        }
...
function marketMyHistoryRequestOnSuccessAfter(url, data, transport) {
  ...
  for (const appId in assets) {
    ...
    for (const key in items) {
      ...
      const assetListingId = (items[key].appid === 730 && items[key].actions) ? items[key].actions[0].link.match(/%20M(.+)A/)[1] : null;

      // if () {
      const obj = Object.assign(
        assetItemObj && !assetItemObj[shortName] ? assetItemObj : {},
  {
          [items[key].name] : {
  ...
  $J(function () {
    ...
    $J('.my_listing_section.market_home_listing_table .market_listing_table_header').each(function () {
      ...
      if ($header.parent().has('div[id^="mylisting"]').length > 0) {
        $J(`<span class="market_listing_right_cell market_listing_minimum can_combine">${SIHLang.market.minimum}</span>`).insertAfter($header.find('.market_listing_my_price'));
      }
      else {
        $header.find('.market_listing_buyorder_qty').before(
